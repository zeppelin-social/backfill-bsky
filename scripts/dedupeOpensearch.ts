...
class CustomHelpers extends Helpers {
	...
	async *scrollDocumentsWithId<TDoc = unknown>(
		...[params, options]: Parameters<Helpers["scrollDocuments"]>
	): AsyncIterable<TDoc & { _id: string }> {
...
function appendFilterPath(
	filter: string,
	params: { filter_path?: string | string[] },
	force?: boolean,
) {
...
async function scanProfiles() {
	for await (
		const document of helpers.scrollDocumentsWithId<ProfileDoc>({ index: PROFILE_INDEX })
	) {
...
const results = await Promise.allSettled([
	scanPosts().catch((e) => console.error("scanPosts failed", e)),
	scanProfiles().catch((e) => console.error("scanProfiles failed", e)),
]);
